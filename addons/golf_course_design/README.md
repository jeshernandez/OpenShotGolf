...
- [Gather Course Data](#gather-course-data)
  ...
  - [Custom Cleanup And File Transformations](#custom-cleanup-and-file-transformations)
...
- [Build Terrain](#build-terrain)
  ...
  - [Coordinate Systems And 1 Meter Pixels](#coordinate-systems-and-1-meter-pixels)
...
### Custom Cleanup And File Transformations
Real course data often needs a small amount of custom cleanup before the dock can use it. Keep the raw file, write the cleaned files beside it, and explain the change in `source_data/SOURCES.md`.

Common cleanup work:

- Split mixed OpenStreetMap exports into separate files. A single export may contain fairways, greens, bunkers, and hole lines. Keep that raw export under a clear name such as `<course>_osm_golf_features.geojson`, then write clean `<course>_holes.geojson` and `<course>_bunkers.geojson` files.
- Keep only the geometry each field expects. Holes must be `LineString` or `MultiLineString`; bunkers must be polygons; boundary must be one course polygon or multipolygon.
- Fix missing or duplicated hole refs before import. The importer sorts by numeric `ref`, so duplicate refs or missing numbers can create confusing hole order.
- Keep the original tee-to-green direction. The first coordinate becomes the tee, and the last coordinate becomes the green or pin.
- Record any manual tracing, deleted duplicate features, or replaced API response in `SOURCES.md`.

Typical transformation chain for a heightmap course:

| Step | Input | Output | Tool | Required? |
| --- | --- | --- | --- | --- |
| Clean map layers | Raw OSM/Overpass export | Boundary, holes, and bunkers GeoJSON | QGIS, Overpass Turbo, `jq`, or `ogr2ogr` | Required only when the source export is mixed or incomplete |
| Find elevation product | Course bounding box | USGS product JSON | Browser or USGS TNM API | Optional API step; useful for repeatability |
| Crop and project elevation | USGS GeoTIFF DEM and boundary | `<course>_dem.tif` | `gdalwarp` or QGIS | Required for the chosen heightmap workflow |
| Fill raster gaps | Prepared DEM | `heightmap_filled.tif` | `gdal raster fill-nodata` | Required for generated heightmap builds |
| Make Terrain3D height input | Filled DEM | `height.exr` | `gdal_translate` | Required for Terrain3D import |
| Build generated zones | Holes and bunkers GeoJSON | GeoPackage and class rasters | `ogr2ogr`, SQLite SQL, `gdal_rasterize`, `gdaldem` | Required only when generated fairway/green/sand painting is enabled |
| Import terrain | `height.exr` and optional class PNG | `Terrain/terrain3d_*.res` | Godot Terrain3D importer | Required by the game |
| Export playable course | `course_design.tres` | `course.gd`, `course.json`, `course.tscn` | Golf Course Design dock or equivalent script | Required by the game |

The API calls in this chain are optional. They make the source path repeatable, but the same cleaned GeoJSON and DEM can be prepared manually in QGIS or through browser downloads. The Terrain3D region files are different: they should be generated by Godot/Terrain3D from the prepared raster. Hand-editing those `.res` files is not a practical replacement for the importer.

Hunter Ranch cleanup example:

- The first local holes file was actually a mixed OpenStreetMap export. It contained 18 hole lines, 60 bunker polygons, 77 fairway polygons, and 24 green polygons.
- The mixed file was preserved as `hunter_ranch_osm_golf_features.geojson`.
- A clean `hunter_ranch_holes.geojson` was written with only 18 hole `LineString` features.
- The mixed export had two `ref=12` hole lines and no `ref=3`. A fresh Overpass hole query supplied the missing `ref=3` way, and the duplicate `ref=12` line was excluded.
- `hunter_ranch_bunkers.geojson` was written from only the 60 bunker polygons.
- The first USGS DEM candidate matched the product footprint but clipped to no valid pixels. The alternate USGS 1 meter DEM product was used after `gdalinfo -stats` confirmed valid elevations.
...
### Coordinate Systems And 1 Meter Pixels
Golf course source files usually start in longitude and latitude (`EPSG:4326`). That is good for web maps, but degrees are not a stable unit for terrain building. One degree of longitude covers a different ground distance depending on latitude, so a raster in degrees does not naturally mean "one Godot unit per meter."

For United States courses, use a local meter-based coordinate system before building terrain. Hunter Ranch is in UTM zone 10, so `EPSG:26910` is the right local CRS for that course. A Fresno course in UTM zone 11 used `EPSG:26911` instead. The exact EPSG code changes by location; the goal is the same: use meters.

When the DEM is projected to `EPSG:26910` with `-tr 1 1`, each raster pixel is one meter wide and one meter tall on the ground. With `Meters to Godot scale = 1.0`, that means one raster pixel becomes roughly one Godot world unit. This affects the game in practical ways:

- Hole coordinates line up with the imported terrain because `gdaltransform` can project tee and pin points into the same meter grid as the DEM.
- Fairway corridors, tee discs, green discs, and bunker polygons use real meter sizes. A `25` meter fairway corridor is painted as about `25` terrain units.
- Terrain3D region files get predictable dimensions. A `1160 x 963` one-meter DEM creates a course area about `1160 m x 963 m` before Terrain3D pads it into regions.
- Shot distances and camera framing are easier to reason about because terrain X/Z coordinates are meter-like.

Alternatives have tradeoffs:

| Choice | What it means | Effect |
| --- | --- | --- |
| `EPSG:4326` degree raster | Pixels are degrees, not meters | Hole alignment and painted zone widths can be distorted or misplaced |
| Wrong UTM zone | Coordinates are projected in meters, but not local to the course | Distances and alignment may drift more than needed |
| `-tr 2 2` | Two-meter pixels | Smaller terrain files and faster builds, but less terrain detail |
| `-tr 0.5 0.5` | Half-meter pixels | More detail, but much larger files and slower Terrain3D import |
| `Meters to Godot scale > 1` | One real meter becomes more than one Godot unit | Course appears stretched in gameplay |
| `Meters to Godot scale < 1` | One real meter becomes less than one Godot unit | Course appears compressed in gameplay |
...
The generated-zone pipeline uses GeoPackage files (`.gpkg`) because they are a compact GIS format backed by SQLite. SQLite is the small local database engine inside each GeoPackage file. The add-on does not use it as a game database. It uses SQLite through `ogr2ogr` so GDAL/OGR can run geometry SQL such as:

- `ST_Buffer(geom, 25)` to turn hole center lines into fairway corridors.
- `ST_StartPoint(geom)` and `ST_EndPoint(geom)` to place tee and green discs.
- `CAST(ref AS INTEGER)` to turn hole refs into numeric class IDs.

This is needed because raster painting works from polygons, not from raw line strings. The SQL step creates those temporary polygons, then `gdal_rasterize` burns their class IDs into `overlay_class.tif`. The class raster is later converted to `overlay_class_ids.png`, which the Godot terrain runner reads while painting Terrain3D control maps.
...
## Common Issues
These issues usually appear during manual cleanup or one-off headless scripts, not during normal dock usage.

### GDScript Type Inference In Export Scripts
If a custom export script reads JSON dictionaries and uses `:=`, Godot may fail with an error like:

```text
Cannot infer the type of "hole_name" variable because the value doesn't have a set type.
```

This happens because values read from `Dictionary` or parsed JSON are untyped variants. The script may know that `item["name"]` is a string, but GDScript cannot prove it when `:=` asks the parser to infer a static type.

Fix it by declaring the type or casting the value:

```gdscript
var hole_name: String = item["name"]
var hole_path: String = "HoleMarkers/" + hole_name
var color: String = TEE_COLORS[tee_index]
var tee_name: String = color + " Tee"
```

Use this only for custom automation. The dock export path already writes `course.gd`, `course.json`, and `course.tscn` through the add-on.

### SQLite Database Is Locked
Generated-zone files are GeoPackages, and GeoPackage is backed by SQLite. SQLite allows many readers but only one writer at a time. If several `ogr2ogr -update -append` commands write to the same `.gpkg` in parallel, one can fail with:

```text
database is locked
```

Run those append commands one after another. Parallel reads are fine; parallel writes to the same GeoPackage are not.

### Product Footprint Covers The Course But The DEM Is Empty
USGS product search results can list a DEM whose footprint covers the course while the actual raster has no valid pixels over the clipped boundary. Check the clipped output with:

```bash
gdalinfo -stats <course>_dem.tif
```

If the stats report no valid pixels, try another product from the same USGS response or inspect both the source raster and boundary in QGIS.
